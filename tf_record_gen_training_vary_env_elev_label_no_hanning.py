...
train_filename = '/nobackup/scratch/Wed/francl/noiseRecords_logspaced_fluctuating_0.5octv_anechoic_convolvedHRIR140_stackedCH_upsampled/testset/train{}.tfrecords'.format(
    version)
...
background_textures = False  # I think this only adds azim/elev metadata to the TFRecord, but the background is still combined
...
def create_feature(subbands, labels=None):
    if background:
        ...
        # Combined background with information on each of the background textures
        feature = {'train/azim': _int64_feature_numpy(labels[0]),
                   # this is a list, azimuth for every individual texture
                   'train/elev': _int64_feature_numpy(labels[1]),
                   # this is a list, elevation for every individual texture
...
def create_record():
    ...
    # SBATCH --array=0-259%100 -> %100 limits simultaneously running tasks to 100
    # version -> 0-259

    metadata_dict = {}
    if background_textures:
        json_fnames = glob(file_path.split('*')[0] + '*.json')
        # All .json files in the same directory as the spatialized sounds
...
if not correct:
    ...
    if not correct:
        ...
        raise TypeError(
            "Data corrupted in {} at record {} and is unrecoverable. File Deleted.".format(train_filename, i))
